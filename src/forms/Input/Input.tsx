...
const Input = ({ label, name, inputProps, placeholder, inputClassName = "", ...props }: IInput) => {
	return (
		<Form.Item label={label} name={name as string} {...props}>
			<TextInput
				className={twMerge(
					"border border-light-blue rounded-md md:rounded-[10px] xl:text-base !h-[33.84px] md:h-14  lg:text-sm bg-light-2 placeholder:text-accent/60 xl:px-5 xl:py-4 py-0 px-4 hover:!shadow-none hover:border-primary active:shadow-none focus:shadow-none",
					inputClassName
				)}
				name={name as string}
